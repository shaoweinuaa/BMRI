function [result,queryIndex,predict_b,unLabelIndexSS]=ActiveLearning(labelIndexSS,data,label,batchSize,maxIteration,rho1,rho2,rho3)
  ...
  TestSample=data(unLabelIndexSS,:);
  TestLabel=label(unLabelIndexSS);
  options = ['-t 2 -c ' num2str(1) ' -g ' num2str(1)];
  model=svmtrain(double(TrainLabel),double(TrainSample),options);
  [predict_b, accuracy, dec] = svmpredict(double(TestLabel), double(TestSample), model);
